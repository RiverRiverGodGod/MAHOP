# Eleventh week: E-commercial Dataset Evaluation & Find the Optimal Parameters for Full-Finetune & Finish All RL Ablation Study for Full-Finetune Setting & Rewritten Introduction and Paper Framework & Cross-validation for Different RAG & Try Some Black-hat Attack to RAG

## API choice

- **Backbone**: Qwen3-1.7B, gemini-2.5-flash-lite-preview
- **Finetune Structure**: LLaMA Factory, Lora, full-param
- **RL Structure**: open-r1, GRPO
- **RAG Generation API**: gpt-4o-mini, gemini-2.5-flash-lite-preview, claude-3-haiku-20240307

## General Abstrct:

### E-commercial Dataset Evaluation

Following an initial LLM filtering for e-commerce queries, we manually deduplicated the data and removed queries masquerading as recommendations, yielding ~2000 queries. We used 1600 for the training set and the remainder for validation. The rules generated by our autorule process differ significantly from RQ's rules. The bolded keywords, which are relevant to e-commerce recommendations, illustrate these differences.

### Rewritten Introduction and Paper Framework

In accordance with last week's discussion with Shanshan, I have logically restructured the introduction and reorganized the paper's framework. Additionally, I have added a brief caption to each heading and subheading.

### Cross-validation for Different RAG

We compared the performance of different RAG models using various rule extraction techniques. We found that although using each RAG's own LLM as its reasoner and extractor produced more differentiated rule sets, the actual GEO Scores were suboptimal. The highest GEO Score was achieved by uniformly applying Gemini as the reasoner and extractor across all RAG models, a method that also surpassed the performance of using the Gemini-rule prompt directly.

### RL and Finetuning

We slightly reduce the learning rate for full-parameter finetuning, and fine that the performance of full-parameter finetuning is indeed better than LoRA, and then we do all the ablation experiments on the finetuning sets.

### Try Some Black-hat Attack to RAG

We currently tried 2 attack ways, but these ways cannot significantly improve GEO scores (only for 0.21~0.22).

## E-commercial Dataset Rules Comparison
...
1. Continue paper writting.

2. Finished the rest experiment.

3. Continuing finding the effective black-hat attack.
