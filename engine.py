...
from model_improvement import RLPnet
import numpy as np
import torch
...
class Engine():
    ...
    def train_one_epoch(self, train_loader, optimizer, criterion):
        ...
        for _,  (batch_x, batch_y) in enumerate(train_loader):
            ...
            # loss = 0.1*loss1 + 0.1*loss2 + 0.1*loss3 +loss4
    ...
    def validate_one_epoch(self, valid_loader, criterion):
        ...
        with torch.no_grad():
            for _,  (batch_x, batch_y) in enumerate(valid_loader):
                ...
                loss = loss1 #loss1 + loss2 + loss3 + loss4
    ...
    def test(self, test_loader):
        ...
        # return rmse_error4.item(), mea_error4.item()
        #
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            ...
            #     break
            # self.logger.info('ALL RMSE : \t\t{:.6f}'.format(rmse))
            # self.logger.info('ALL MEA 电视: \t\t{:.6f}'.format(mea))
            if rmse<100 and mea<100:
                self.logger.info('ALL RMSE 冰箱: \t\t{:.6f}'.format(rmse))
                self.logger.info('ALL MEA 冰箱: \t\t{:.6f}'.format(mea))
