...
class RLPnet(nn.Module):
    def __init__(self, feature_size, predict_w, use_seaon, use_trend):
        ...
        self.all_tower = Series_Embedding(self.feature_size, self.trend_embedding_size)
        ...
        self.gate1 = nn.Linear(2, 1)
        self.gate2 = nn.Linear(2, 1)
        self.gate3 = nn.Linear(2, 1)
        self.gate4 = nn.Linear(2, 1)

    def forward(self, x: torch.Tensor):
        near_seq, season_seq, trend_seq = x
        sum_seq=torch.cat([near_seq, season_seq, trend_seq], dim=1)
        sum_embedding=self.all_tower(sum_seq)
        # print(sum_seq.shape)
        ...
        output1 = self.decoder1(self.tocket1(tower_sum).squeeze(2)+sum_embedding)
        output2 = self.decoder2(self.tocket2(tower_sum).squeeze(2)+sum_embedding)
        output3 = self.decoder3(self.tocket3(tower_sum).squeeze(2)+sum_embedding)
        output4 = self.decoder4(self.tocket4(tower_sum).squeeze(2)+sum_embedding)
