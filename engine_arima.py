...
class Engine_ARIMA():
    ...
    def get_data(self, cate, flag):
        ...
        df_raw = pd.read_csv(self.args.data_path).reset_index(drop=True)[:-15]
        cate_data = df_raw[cate].values
        if flag == 'train': return cate_data[ :-(28 + self.args.predict_seq_window)]
        else: return cate_data[-(28 + self.args.predict_seq_window):]
